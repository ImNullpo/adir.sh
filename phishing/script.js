...
async function scanUrl() {
    ...
    if (!inputText) {
        displayResult("ERROR: No input provided. Please enter a URL.", "error");
        return;
    }
    ...
    if (!urlToCheck) {
        if (inputText.includes('.') && !inputText.includes(' ') && inputText.length > 3) {
            ...
            displayResult("ERROR: Could not find a valid target parameter.", "error");
            return;
        }
    }
...
`;
                // Treat unknown as potentially dangerous in this context
        ...
        // Parse VirusTotal Stats from proxy
...
function displayResult(message, type) {
    ...
    // Apply specific styles based on archive aesthetic
    if (type === 'danger') {
        ...
        // inline error style
        resultArea.style.borderColor = "#ff5555";
        resultArea.style.color = "#ff5555";
    }
}
